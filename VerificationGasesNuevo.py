	...
	def Read_Verification(self,*args,**kwargs):
		...
		for tipofile in self.C.keys():
			...
			for est in self.estaciones:
				if (self.var in ['NOx','SO2','Ozono']) and (est!='BEL-USBV'): # or (est!='BEL-USBV' and self.var=='Ozono'): # Aquí se debe agregar la regla para el CO que antes estaba en el POLI si se vuelve a intalar... or (est!='MED-PJIC' and self.var=='CO')
